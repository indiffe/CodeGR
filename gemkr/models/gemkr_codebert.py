...
class GEMKRCodeBERTDSI(BaseModel):
    ...
    生成侧关键修复：
      (1) eos_token_id 只用 </DOCID>（单个 int），避免 beam_search list-eos 问题
      (2) allowed token 集合覆盖 tokenizer 的“数字piece”（含带空格的数字、0串等），且不允许 eos/pad/bos
      (3) min_new_tokens=8，避免直接生成 </DOCID> 导致空输出
    """
    ...
    def _build_allowed_digit_ids(self):
        """
        ✅ 关键修复：不要假设数字一定是“单token”。
        我们通过多种字符串（含前导空格、0串等）收集出 tokenizer 实际用到的 piece，
        然后禁止所有 special（bos/eos/pad/unk）。
        """
        ...
        # 禁止 special ids（尤其 </s> 会导致你看到空/全special）
    ...
    def generate(self, samples, **gen_kwargs):
        ...
        # ✅ 关键修复(1)：只用 </DOCID> 作为 eos（单个 int，不要 list）
        end_id = tok.convert_tokens_to_ids("</DOCID>")
        if end_id is None or end_id < 0:
            raise ValueError("Cannot find token id for </DOCID> (did you add_special_tokens?)")
        ...
        # ✅ 防止直接输出 </DOCID> 结束（否则 eval decode 为空）
        ...
        # ✅ 关键修复(2)：允许集合是“数字piece + </DOCID>”，且不含 eos/pad/bos
        ...
        # 如果外面误传 max_length（总长），把它当成 max_new_tokens
